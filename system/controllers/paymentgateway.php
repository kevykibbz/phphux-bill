...
$action = alphanumeric($routes[1], '_-');
$ui->assign('_admin', $admin);
switch ($action) {
    case 'delete':
        $pg = alphanumeric($routes[2], '_-');
    ...
    default:
        ...
        if (file_exists($PAYMENTGATEWAY_PATH . DIRECTORY_SEPARATOR . $action . '.php')) {
            $ui->assign('_title', ucwords(str_replace(['_', '-'], ' ', $action)) . ' Settings');
            include $PAYMENTGATEWAY_PATH . DIRECTORY_SEPARATOR . $action . '.php';
            $func_name = str_replace('-', '_', $action);
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                if (function_exists($func_name . '_save_config')) {
                    call_user_func($func_name . '_save_config');
                } else {
                    $ui->display('admin/404.tpl');
                }
            } else {
                if (function_exists($func_name . '_show_config')) {
                    call_user_func($func_name . '_show_config');
                } else {
                    $ui->display('admin/404.tpl');
                }
            }
