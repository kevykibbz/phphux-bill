...
switch ($action) {
    ...
    default:
        ...
        if ($name != '') {
            ...
            $d = ORM::for_table('tbl_odps')->where_like('name', '%' . $name . '%')->offset($paginator['startpoint'] ?? 0)->limit($paginator['limit'] ?? 10)->order_by_desc('id')->find_many();
        } else {
            $paginator = Paginator::build(ORM::for_table('tbl_odps'));
            $d = ORM::for_table('tbl_odps')->offset($paginator['startpoint'] ?? 0)->limit($paginator['limit'] ?? 10)->order_by_desc('id')->find_many();
        }
